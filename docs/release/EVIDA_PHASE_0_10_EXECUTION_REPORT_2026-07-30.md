...
| 5. Auth/RBAC/tenant | PASS code and local tests / BLOCKED live IdP | Per-endpoint permissions, HTTPS issuer, MFA `amr`/`acr`, role allowlist and tenant tests are green; live IdP smoke still requires the target |
...
- Pilot/startup contracts: 5 files, 17 tests passed.
- Spring: 175 tests passed, 0 failures/errors, 3 environment-dependent skips.
...
2. Configure the real OIDC tenant, MFA, invited users and role claims; execute `test-production-identity.ps1`.
3. Provision the HTTPS domain, certificates, firewall, secrets and encrypted persistent volumes; deploy and run `test-live-edge.ps1`.
4. Sign digest-pinned release images with the organisation's trusted identity and run cosign verification.
5. Complete native Windows picker and managed-workstation smoke.
6. Sign the pilot agreement/DPA and collect Engineering, Product, Security/Privacy, IT and data-owner approvals against one commit.
7. Run phase 9 only after the real-client-data gate returns `PASS`; start with one approved document and retain a stop decision.
8. Complete post-pilot production capabilities: monitoring/alerts, customer administration, support, billing and SLA/terms.
